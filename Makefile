# Makefile for publishing timecopilot-chronos-forecasting to PyPI

# Python version to test with
PYTHON_VERSION = 3.13

# Import statement to test
IMPORT_STATEMENT = from chronos import BaseChronosPipeline
...
after-publish-test:
	uv run --isolated --no-project -p $(PYTHON_VERSION) --with timecopilot-chronos-forecasting -- python -c "$(IMPORT_STATEMENT)"
